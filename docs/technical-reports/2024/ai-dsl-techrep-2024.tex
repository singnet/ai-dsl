...
servers, but the description itself of the SingularityNET marketplace
...
NEXT: add special treatment data only lambda abstraction.
...
\item Combinatory logic is also equally powerful to describe
  ...
  is also called \emph{tacit programming}~\cite{enwiki:1283676462}.
\end{itemize}
...
future avenue of research but we have not explored it in this report.

\subsection{Pruning Techniques}
In order to prune the search we have experimented with a few things.
\subsubsection{Limit the Flow of Information to Data, as Opposed to
  Functions} This means that on the wires on the composition only data
can transit, not functions.  To realize this we implement a constraint
directly inside the backward chainer entry handling lambda
...
\mintinline{scheme}{if} conditional but the else branch is replaced by
\mintinline{scheme}{Empty}, resulting in pruning the evaluation if the
condition is false.  In this case the condition
\begin{minted}{scheme}
(or (not &OnlyData) (not (is-arrow-type $prms)))
\end{minted}
expresses that if the \mintinline{scheme}{&OnlyData} hyper-parameter
is \mintinline{scheme}{True}, then the premise
\mintinline{scheme}{$prms}, the type of the variable scoped by the
lambda abstraction, must not be an arrow type, thus must be a data
type.
...
the identity function.  This latest reduction rule is specialized to
the \mintinline{scheme}{machine-translation.translate} service and
indicates that translating a text from a source language to an
identical destination language is equivalent to doing nothing. We have
implemented over a dozen reduction rules for each experiment.  The
result is a notable, sometimes dramatic, speed-up in most experiments,
as well as a reduction in the number of resulting candidates.
...
data, using reduction rules as well as a carefully selected uncurried
combinator set leads to efficient synthesis of AI service
compositions.
...
moves on to bluebird and phoenix combinators.  Even without reduction,
and still using curried combinators, the best solution is found in
1m42s.  Enabling reduction in experiment $\texttt{xp}_9$ brings that
down to 42.7s.  Moreover the number of candidates goes from 29 to 1,
