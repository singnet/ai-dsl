...
Table~\ref{tab:benchmarks} provides a benchmark summary for some of
our most meaningful experiments.
\begin{table}[h]
  \centering
  \caption{Benchmarks sorted increasing order of difficulty.  All file
    paths are relative to path
    \texttt{experimental/ai-service-composition} from the root folder
    of the AI-DSL repository~\cite{TODO}.}
  \label{tab:benchmarks}
  \begin{tabular}{|l|L|}
    \hline
    Experiment ID & $\texttt{xp}_1$ \\
    \tabdashline & \tabdashline
    Relative path & \texttt{recognize-speech-emotion} \\
    \tabdashline & \tabdashline
    ...
    \tabdashline & \tabdashline
    Description & Like $\texttt{xp}_1$ but lambda abstraction is replaced
    by the bluebird combinator \\
    \tabdashline & \tabdashline
    Parameters &  \{Reduction: Disabled \} \\
    \tabdashline & \tabdashline
    Results & TODO \\
    \hline
...
Thanks to Remy Clarke for mentioning APL and BQN to me which led me to
learn about tacit programming.  Thanks to Douglas Miles for making
MeTTaLog which has been tremendously helpful for running some
experiments.  Thanks to the SingularityNET Foundation for funding this
work.
