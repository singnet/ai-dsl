...
\begin{table}[h]
  ...
  \begin{tabular}{|l|l|}
    ...
    \hline
    \parbox{1cm}{\vspace{-45pt} \centering Diagram} &
...
\begin{table}[h]
  ...
  \begin{tabular}{|l|l|}
    ...
    \hline
    \parbox{1cm}{\vspace{-60pt} \centering Diagram} &
...
recognizing emotion in speech, involving only two services to be
sequentially combined.  In these experiments no ontology has been
used, instead the Protobuf types are directly used to specify the
input and output of the desired composition.  The query submitted to
the backward chainer in all cases is
\begin{minted}{scheme}
(: $prg (-> speech2text-en.Audio text-emotions.Output))
\end{minted}
The benchmarks are detailed in table~\ref{tab:rse}.  The first three
experiments involving lambda abstraction deliver poor (over 19 minutes
of run-time in the best case).  The poor performances are due to
lambda abstraction requiring more depth to describe the desired
...
of functions, the time is considerably reduced, but is still large
(23m40s).  Pruning the search space by applying reduction rules
(experiment $\texttt{xp}_3$) reduces run-time a bit further (19m6s).
Notably the number of resulting candidates is lower as well, going
from 44 to 20.  This is good because less candidates means less work
to the user (or whatever upstream process will use the output of AI
service composition) to carry out the final selection.  When using
MeTTaLog, results for $\texttt{xp}_3$ down to 1m21s, which is far
better than 19m6s with hyperon-experimental, but still unacceptable
for such simple AI service composition.  The best solution synthesized
by experiments $\texttt{xp}_1$, $\texttt{xp}_2$ and $\texttt{xp}_3$ is
given below.
\begin{minted}{scheme}
...
(: (\ (: z speech2text-en.Audio)
  ...
  (-> speech2text-en.Audio text-emotions.Output))
\end{minted}
It is incorrect because it never involves the
\mintinline{scheme}{text-emotions.recognize} service.  Instead it
tricks the type by directly constructing an emotion from the text
output by \mintinline{scheme}{speech2text-en.s2t} using the
\mintinline{scheme}{text-emotions.MkOutput} constructor.  This is a
perfect example of an under-specified formal description.  Obviously,
\mintinline{scheme}{text-emotions.MkOutput} should not be allowed to
construct an emotion from arbitrary text.  This is where the ontology
comes into play and will be explored in the subsequent experiments.
...
not function, transit between services.  Here enabling reduction
(experiment $\texttt{xp}_6$ had no impact on synthesis time however it
did reduce the number of resulting candidates from 3 to 2.  The best
solution that was synthesized by experiments $\texttt{xp}_5$ and
$\texttt{xp}_6$ is given below.
\begin{minted}{scheme}
(: (. text-emotions.recognize
      (. text-emotions.MkInput
         (. speech2text-en.Text.text
...
\begin{table}[h]
  \centering
  \caption{AI service composition benchmarks for recognizing emotion
    in speech.  The files involved to reproduce the experiments can be
    found under the folder
    \texttt{experimental/ai-service-composition/recognize-speech-emotion/}
    from the root folder of the AI-DSL repository~\cite{TODO}.  The
    results are indicated in time to synthesize the AI service
    composition.  }
  \label{tab:rse}
  \begin{tabular}{|l|L|}
    \hline
...
\subsection{English To Chinese Song (Scaled Down)}
NEXT
