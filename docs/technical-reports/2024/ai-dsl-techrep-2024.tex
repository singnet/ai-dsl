...
from 44 to 20, in spite of having provided only two reduction rules
\begin{minted}{scheme}
...
(= ((\ (: (s z) $t) z) $x) z)
\end{minted}
It is good because less candidates means less work to the user or
whatever upstream process will use the output of AI service
composition.  When using MeTTaLog, results for $\texttt{xp}_3$ goes
down to 1m21s, which is far better than 19m6s with
hyperon-experimental, but still unacceptable for such simple AI
...
\label{sec:etcs}
We first experiment with a scaled down version of the English to
Chinese Song AI service composition.  None of our NEXT
