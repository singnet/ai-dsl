...
\item Integrate a form of attention allocation to restrict the set of
  ...
  remains to be determined.  It could be that some attention
  allocation mechanism could be designed specifically for the AI-DSL,
  or a more general-purpose one such as ECAN~\ref{TODO} or
  SINE~\ref{TODO} could be used.
...
\item Incorporate uncertainty of resources and outcomes in the formal
  description using a probabilistic logic such as PLN~\cite{TODO}.
\item How to build the ontology and enrich Protobuf specifications to
  leverage such ontology.
\item NEXT: Use infer grained language that captures not just functional
  semantics but also run-time behavior.
\item NEXT: inference control.
\end{enumerate}
