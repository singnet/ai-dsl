...
\cite{Witherell2009} introduces FIDOE, a tool for analyzing and
propagating changes across overlapping ontologies according to
predefined inference rules.  It could be relevant to the ontology
part, they mention standards such as SUMO~\cite{pease_standard_2009}
and SWRL~\cite{SWRL} as well.\nil{Add Euzenat, J, Isaac, A, Meilicke,
  C, Shvaik, P, Stuckenschmidt, H, Šváb, O, Svátek, V, van Hage, W,
  Yatskevich, M, “Results of the Ontology Alignment Evaluation
  Initiative 2007”, The Second International Workshop on Ontology
  Matching, Busan Korea, 2007.  As well as Lee, M., Jung, M., Suh, J.,
  “Semantic Mapping Based on Ontology and Bayesian Network and its
  Application to CAD and PDM Integration,” 2006 ASME Computers and
  Information in Engineering Conference, Philadelphia, PA, September
  2006 DETC2006-99419, in Github issue}
...
In Section \ref{realized_function} we describe experiments for
...
Section \ref{network_idris_ai_services} we describe the implementation
of a network of trivially simple AI services implemented in Idris, and
use Idris compiler to type check if they can properly connect to each
other.  Finally, in Section \ref{aidsl_registry} we describe the
...
Section \ref{realized_function}, which was fully achieved in this
...
experiment, the objectives were met.  See Section \ref{aidsl_registry}
...
\chapter{Conclusion}
Below is the list of what we have accomplished so far and what are the
next steps to bring us closer to the completion of the AI-DSL.  We
have
\begin{itemize}
\item Experimented with Idris to formalize and reason about realized
  function attributes such as costs and quality, see
  \ref{realized_function}.  We have done so in a limited manners, only
...
\item \textbf{Dependent Types}: expressive type system that allows to
  express types depending on values.  Instead of being limited to
  constants such as \texttt{Integer} or \texttt{String}, dependent
  types are functions that take values and return types, usually
  directly expressed as a term containing free variables such as,
  \texttt{Vect n a}, representing the class of vectors of size
  \texttt{n} containing elements of type \texttt{a}.
\end{itemize}
