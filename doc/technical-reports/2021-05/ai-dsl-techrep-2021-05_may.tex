...
same domain model of \textit{Fake News Warking} application. It also clarifies
...
Within the domain model of \textit{Fake News Warning} application (Figure 
...
search and match them, involving ability for an AI Service to choose its dependent
...
\begin{figure}[H]
	...
	dependencies of \textit{uclnlp} SNet AI service instance of \textit{Fake News Warning} 
	application within SUMO ontology.}
\end{figure}

	\item Note, that the correctness of type dependencies of decentralized components of 
	AI-DSL Ontology (\textit{uclnlp}) will be checked against centralized
	...
	think about reasonable way to accommodate them in the system. A possibility 
	is to include information about the version of global components of ontology when communicating
	decentralized components between each other, as suggested by \cite{YvesHellenschmidt2002}.
	
\kabir[inline]{finished here 2021-06-08 03:22}
...
\begin{enumerate}
    ...
    \item Porting idris1 code to idri2 code and inclusion of dependent types 
\end{enumerate}

Work in progress:
\begin{enumerate}
\item implement an AI model that can learn the best loss function for a given task in my case fake news classifier taking advantage of idris’s dependent types
\end{enumerate}

\section{Fake News Classifier}
\subsection{Description}
The idea is to develop an NLP model using word embedding and train it with kaggle dataset \cite{Kaggle_Datasets} 
I implemented a word embedding model based on an article shared by eligijus bujokas on medium  \cite{Word_embedding}.Hence, given a dataset or a csv file containing these lines for instance.
...
The goal is to make the computer understand the relationship between words in a sentence. Therefore, we want to create data points for all words in a sentence such that we have :-
(The, future)(The, king) (future, king) (future,The)..etc 

What we did here is that we took each word and created pairs between 2 words adjacent to it from its left and it's right. For example the word "King" in sentence 1 has 2 words on its left which are "future" and "The" and 2 words on its right "is" and "the". Therefore we have.  (king,future) (king,The) (king,is) (king,the) 
as our data points. "King" here is our focus word and the points created are called context. For this example we took the context as only 2 words but we can make it 4,5,6... etc as we wish. This can be elegantly implemented in Haskel like so.
...
Before we make word pairs ,however, we perform some common dataset  pre-processing such as removing stop words and creating dictionaries
\begin{minted}
...
Finally, we pass it to our model which has one Deep Neural Network of (n,n) Shape. We apply softmax activation to our output and compute cost using cross entropy loss function. model here returns newly learned weights
...
\subsection{Objectives and Achievements}
The objective of the work was to explore idris better and get familiar with it. Since idris adopts Haskel's syntax I thought it would be better I got familiar with Haskel first. I decided to study it by reading the learn you haskel for good by "" which is the best open source well written resource for haskel. Writing the fake News classifier with haskel was just my attempt of understanding haskel better i.e is not related  to the task. Both the idris and Haskel model accepts a list of list of Strings which represents a dataset and return weights predicted by the models. The function that tests the model's prediction over a test set is not implemented yet but can be added in the future.

\subsection{Future work}
it's worth noting that the above models return the learned weights and not the accuracy of prediction or losses like in classical machine learning tasks. Therefore, depending on the priority given to it, we can  add these functionalities for the work to be fully complete.

\section{Code rewriting using dependent types}
\section{Work in progress: Ai model that can learn suitable cost function for a given AI task}
