...
To understand what is going on I recommend to run that code while
tracing the calls.  Below is the trace for the last query that I have
commented {\footnotesize
\begin{minted}{scheme}
  ;; Original call (base case entry)
  (bc-bas (S (S Z)) (: $prg (-> $intput Bool)))
  ;; Original call (recursive step entry)
  (bc-rec (S (S Z)) (: $prf#253 (-> $intput Bool)))
  ;; First recursive call on function (base case entry)
  (bc-bas (S Z) (: $prfabs#250 (-> $prms#254 (-> $intput Bool))))
  ;; First recursive call on argument (recursive step entry)
  (bc-rec (S Z) (: $prf#858 (-> $prms#254 (-> $intput Bool))))
  ;; Second recursive call on function (base case entry)
  (bc-bas Z (: $prfabs#855 (-> $prms#859 (-> $prms#254 (-> $intput Bool)))))
  ;; Second recursive call on argument (base case entry)
  (bc-bas Z (: $prfarg#856 (-> $b#1753 Bool)))
  ;; First recursive call on argument (base case entry)
  (bc-bas (S Z) (: $prfarg#251 (-> $intput String)))
  ;; First recursive call on argument (recursive step entry)
  (bc-rec (S Z) (: $prf#2654 (-> $intput String)))
  ;; Second recursive call on function (base case entry)
  (bc-bas Z (: $prfabs#2651 (-> $prms#2655 (-> $intput String))))
\end{minted}
}
NEXT: arrange in tree like trace
