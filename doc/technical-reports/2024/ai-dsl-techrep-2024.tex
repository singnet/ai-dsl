...
\subsection{Locally Modifiable Space}
...
\mintinline{scheme}{(s (s z))} represents the third, and so on.
...
(: bc (-> $a                            ; Knowledge base space
          ...
          $b))                          ; Result
\end{minted}
Two additional arguments are passed
\begin{itemize}
\item An environment, typed \mintinline{scheme}{(List $b)}, holding
  ...
  knowledge base space argument statically holds the rest of the
  theory.
\item A De Bruijn index, typed \mintinline{scheme}{DeBruijn}, to be
  used as variable in the next encountered $\lambda$ abstraction.
\end{itemize}
...
doubles a number, against its type \mintinline{scheme}{(-> Number Number)}.
...
\end{minted}
At this point the backward chainer needs to unpack
...
free to play with the implementation provided in~\cite{TODO}.
