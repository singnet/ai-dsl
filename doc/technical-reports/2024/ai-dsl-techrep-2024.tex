...
\end{minted}
\end{small}
One may observe that in this ontology example all types eventually
...
of type \mintinline{scheme}{String}, the type coercion may actually
not need to take place at run-time.  Type coercion is, most of the
time, only here to make sure the AI service composition is consistent
with its AI service specifications.  One may also observe that some
types are parameterized, such as \mintinline{scheme}{(TextIn $l)},
which represents a text in a certain language \mintinline{scheme}{$l}.
Having parameterized types in the ontology will turn out to useful
while reasoning about dependent types.

NEXT: write a tool to display ontology.

NEXT: add example of AI service type signature before and after
ontology.
