...
: H$, results in proof $$((\lambda x. c)\ h) : C$$ perfectly emulating
the behavior of the modus ponens rule.  Thus one may conclude that the
backward chainer presented so far is essentially a generic
implementation of modus ponens.  It makes sense therefore that one
would want to support its dual operation, lambda abstraction.  NEXT:
combinatory logic as alternative.
