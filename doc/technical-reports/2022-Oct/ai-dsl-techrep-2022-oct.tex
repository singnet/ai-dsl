...
their system also uses input-output examples to further narrow down
the search.  TTN-based synthesis works well on \emph{Concrete Data
  Types}, like \texttt{List Int}, but not so well on \emph{Abstract
  Data Types}, like \texttt{List a} where \texttt{a} is a variable.
To address that a variation of TTN has been introduced called
\emph{Abstract Transition Network} (ATN)~\cite{Guo2020}.  Such
...
compositions given a type signature, instead of standalone functions.
