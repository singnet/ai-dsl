...
import service.proto_spec.compo2_service_pb2_grpc as grpc_compo2_grpc
from service.proto_spec.compo2_service_pb2 import Result

from service import registry

logging.basicConfig(level=10, format="[%(asctime)s] [%(levelname)s] [%(name)s] %(message)s")
log = logging.getLogger("compo2_service")

"""
Simple service to test Idris-based specification checking for
services running over gRPC.

Implements a composition of (incrementer . twicer . halfer) by sequentially
invoking incrementer_service, twicer_service and halfer_service
"""

# Create a class to be added to the gRPC server
# derived from the protobuf codes.
class Compo2Servicer(grpc_compo2_grpc.Compo2Servicer):
    def __init__(self):
        self.argument = 0
        self.result = 0

        # Type check
        cmd = ["idris2", "Compo2.idr", "--check"]
        ...
        log.debug("Compo2Servicer created")
    ...
    def compo2(self, request, context):
        self.argument = request.argument
        self.result = Result()

        # Python compo2 implementation
        try:
            # incrementer . twicer . halfer
            self.result.value = \
                self.call_incrementer_svc(
                    self.call_twicer_svc(
                        self.call_halfer_svc(self.argument)))

            # Return compo2 result
            log.debug("compo2 {} = {}".format(self.argument, self.result.value))
            return self.result
...
def serve(max_workers=10, port=7777):
    ...
    grpc_compo2_grpc.add_Compo2Servicer_to_server(Compo2Servicer(), server)
