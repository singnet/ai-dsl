...
my_min_eq : Ord a => (x, y : a) -> Either (my_min x y = x) (my_min x y = y)
my_min_eq x y with (x < y)
  _ | True = Left Refl
  _ | False = Right Refl

||| Proof that my_min is commutative
my_min_commutative : Ord a => (x, y : a) -> my_min x y = my_min y x
my_min_commutative x y with (x < y) proof eq1 | (y < x) proof eq2
  _ | True | False = Refl
  _ | False | True = Refl
...
my_min_ngt : Ord a => (x, y : a) -> (x < my_min x y = False, y < my_min x y = False)
my_min_ngt x y with (x < y) proof eq
  _ | True = (lt_irreflexive, lt_asymmetric eq)
  _ | False = (eq, lt_irreflexive)

||| Proof that my_min x y is equal to or lower than x and y
my_min_le : Ord a => (x, y : a) -> (my_min x y <= x = True, my_min x y <= y = True)
my_min_le x y with (x < y) proof eq
...
my_min_with : Ord a => (x, y : a) -> (m : a ** (m <= x = True, m <= y = True))
my_min_with x y = (my_min x y ** my_min_le x y)
...
min_element_le : Ord a => (xs : Vect (S n) a) -> All (\x : a => (min_element xs) <= x = True) xs
min_element_le (x :: []) = le_reflexive :: []
min_element_le (x :: (y :: ys)) = head :: tail
  where head : (min_element (x :: (y :: ys))) <= x = True
        head = fst (my_min_le x (min_element (y :: ys)))
...
min_element_with : Ord a => (xs : Vect (S n) a) -> (m : a ** All (\x : a => m <= x = True) xs)
