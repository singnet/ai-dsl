...
public export
lt_connected : Ord a => {0 x, y : a} -> x < y = False -> y < x = False -> x = y
...
public export
le_strongly_connected_imp : Ord a => {0 x, y : a} -> x <= y = False -> y <= x = True
